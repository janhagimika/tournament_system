...
class TretiPohled : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.pohled_treti)
        ...
        bottomNavigationView.setSelectedItemId(R.id.strikers)

        val tabulka = findViewById<TableLayout>(R.id.tableOfStrikers)
        var helper = MyDBHelper(applicationContext)
        var db = helper.readableDatabase
        val dbHandler: MyDBHelper= MyDBHelper(this)
        val query = "SELECT NAME, TEAM, SUM(GOALS) AS GOALS, MATCHID FROM STRIKERS GROUP BY NAME ORDER BY GOALS DESC"
